...
def categorize_accidents(descriptions, batch_start, system_prompt, filename, session_id, ctx=None, st_bar=None):
    ...
    input_text = "Sr.No. Description\n" + "\n".join(
        [f"{batch_start + i+1}. {desc[0]}" for i, desc in enumerate(descriptions)]
    )

    try:
        prompt_name = "categorize_accident_category_standard"
        tags = [filename]
...
def process_and_parse_descriptions(df, column_name, system_prompt, filename, session_id, batch_size):
    ...
    categorization_dict = {
        item["SNo"]: item["Category"] for item in all_categorized
    }
    
    valid_categories = (
        "Struck while Parked",
        "Insured hit in rear",
        "Insured hit by other vehicle",
        "Other vehicle lost control",
        "Cut in and out sideswipe",
        "Intersection",
        "Insured backing",
        "Hit parked vehicle",
        "External object",
        ...
        "Head-on collision",
        "Hit animal",
        "Hit claimant in rear",
        "Hit pedestrian/cyclist/other",
    ...
    def get_valid_category(sno):
        ...
        return matches[0][0] if matches else category

    df["AccidentCategory"] = df["SNo"].apply(get_valid_category)
    return df

def run():
    if "accident_gpt_processed_df" not in st.session_state:
        st.session_state.accident_gpt_processed_df = None
    if "accident_gpt_current_file" not in st.session_state:
        st.session_state.accident_gpt_current_file = None

    uploaded_file = st.file_uploader("Choose a CSV file", type="xlsx")
    if uploaded_file is not None:
        if st.session_state.accident_gpt_current_file != uploaded_file.name:
            st.session_state.accident_gpt_processed_df = None
            st.session_state.accident_gpt_current_file = uploaded_file.name
        ...
        if st.button("Process"):
            ...
            with st.status("Predicting Categories..."):
                ...
                st.session_state.accident_gpt_processed_df = processed_df
            
        if st.session_state.accident_gpt_processed_df is not None:
            df = st.session_state.accident_gpt_processed_df
