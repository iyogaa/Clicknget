...
def process_and_parse_descriptions(df, column_name, system_prompt, filename, session_id, batch_size):
    ...
    #     "Exposure to Contagious or Blood-Borne Disease",
    #     "Fracture",
    #     "Inflammation",
    #     "Inhalation",
    ...
    df["Category"] = df["SNo"].map(get_valid_category)
...
def run():
    ...
    if uploaded_file is not None:
        ...
        column_name = st.selectbox(
            ...
            options=["LossDescription", "ResultingInjuryDesc"]
        )

        custom_prompt = st.text_area("Enter your custom prompt", value="")
        if not custom_prompt.strip():
            st.error("Custom prompt cannot be empty. Please enter your custom prompt.")
            return
        ...
        if st.button("Process"):
            ...
            # with open("highview_injury_gpt/prompt.txt", "r") as file:
